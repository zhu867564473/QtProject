...
void MagicPool::updateMove()
{
    ...
    else
    {
        ...
        m_fish->move(QPointF(p.x(), p.y()).toPoint());
        update();
    }
}
...
void MagicPool::paintEvent(QPaintEvent *event)
{
...
void MagicPool::mousePressEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton){
        moveFish(event->x(),event->y(),true);
    }
    QWidget::mousePressEvent(event);
}

void MagicPool::timerEvent(QTimerEvent *event)
{
    if(getRandom() > 0.6 && moving()){
        randomMove();
    }
    QWidget::timerEvent(event);
}

void MagicPool::showEvent(QShowEvent *event)
{
    randomMove();
...
void MagicPool::randomMove()
{
    auto r_x = getRandom() * width();
    auto r_y = getRandom() * height();
    moveFish(r_x, r_y, false);
}

double MagicPool::getRandom() const
{
    return (static_cast<double>(qrand() % 1000000000) / 1000000000);
}

QColor MagicPool::getFishBodyColor() const
{
    return m_fish->getBodyColor();
}

void MagicPool::setFishBodyColor(const QColor &fishBodyColor)
{    
    m_fish->setBodyColor(fishBodyColor);
}

QColor MagicPool::getFishFinColor() const
{
    return m_fish->getFinColor();
}

void MagicPool::setFishFinColor(const QColor &fishFinColor)
{
    m_fish->setFinColor(fishFinColor);
}

QColor MagicPool::getFishTailColor() const
{
    return m_fish->getTailColor();
}

void MagicPool::setFishTailColor(const QColor &fishTailColor)
{
    m_fish->setTailColor(fishTailColor);
}

QColor MagicPool::getFishHeadColor() const
{
    return m_fish->getHeadColor();
}

void MagicPool::setFishHeadColor(const QColor &fishHeadColor)
{    
    m_fish->setHeadColor(fishHeadColor);
}
