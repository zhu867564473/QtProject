...
MagicFish::MagicFish(QWidget *parent)
    : QWidget(parent),
      ...
      m_startFin(false),
      m_paintPoint(false),
      m_headColor(QColor(244, 92, 71, m_headAlpha)),
      m_bodyColor(QColor(244, 92, 71, m_bodyAlpha)),
      m_finColor(QColor(244, 92, 71, m_finAlpha)),
      m_tailColor(QColor(244, 92, 71, m_headAlpha))
{
...
{
    QSharedPointer<QPainter> painter(new QPainter(this));
    painter->setRenderHint(QPainter::Antialiasing);
    QPointF middle_pos = QPointF(width() / 2, height() / 2);
    m_headPos = calcPoint(middle_pos, m_bodyHeight / 2.0, m_mainAngle);
    paintMyPoint(painter.data(), m_headPos);
    paintMyPoint(painter.data(), middle_pos);
    painter->setPen(Qt::NoPen);

    painter->setBrush(QBrush(m_headColor));
    ...
    paintMyBody(painter.data(), end_pos, m_fishRadius * 0.7, 0.6, angle);
    QPointF left_fin_pos = calcPoint(m_headPos, m_fishRadius * 0.9, angle + 110);
    paintMyFishFins(painter.data(), left_fin_pos, true, angle);
    QPointF right_fin_pos = calcPoint(m_headPos, m_fishRadius * 0.9, angle - 110);
    paintMyFishFins(painter.data(), right_fin_pos, false, angle);
}
...
void MagicFish::paintMyFishFins(QPainter *painter, const QPointF &pos, bool is_left, qreal father_angle)
{
    ...
    painter->save();
    painter->setBrush(QBrush(m_finColor));
    painter->drawPath(path);
    painter->restore();
}
...
void MagicFish::paintMyTail(QPainter *painter, const QPointF &pos, qreal length, qreal max_w, qreal angle)
{
    ...
    path.lineTo(pos);
    painter->save();
    painter->setBrush(QBrush(m_tailColor));
    painter->drawPath(path);
